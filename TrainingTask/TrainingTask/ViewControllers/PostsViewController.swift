...
//            }
			APIrequests().request(route: PostRoute(), parser: PostParser()) { (result, error) in
				if error != nil {
...
//                }
//            }
			cell.postUserNameLabel.text = postArray[indexPath.row].name
			cell.postTextLabel.text = postArray[indexPath.row].text
			let image = UIImageView()
			for eachPost in postArray {
				image.load(url: eachPost.image) {
